...
void PointManager::getFullList(RESULT_VECTOR &out)
{
    ...
    if( !isEmpty() ) {
        for( ARPOINT_MAP::const_iterator itor = m_container.cbegin() ; itor != m_container.cend() ; ++itor ) {
            ...
            for (auto temp = itor->second.begin(); temp != itor->second.end(); ++temp) {
                point_type_id_t aPt = {(*temp).x, (*temp).y, (*temp).z, static_cast<float>(itor->first)};
                full.insert( full.end(), aPt);
            }
        }
    }
